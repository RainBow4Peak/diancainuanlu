...
void HYM8563_Read_time() //采用写地址读数据模式
{
    ...
    IIC_Stop();//产生一个停止条件
    if(sysParam.Seconds>=0x80)	//VL标志 vl=1 电压过低 不保证准确时钟
    {   //可重置时钟
        sysParam.Seconds&=0x7f;
    }
		    sysParam.Hour_Low=(sysParam.Hour&0x0f);  //小时的个位
    sysParam.Hour_High=((sysParam.Hour>>4)&0x0f); //小时的十位
    sysParam.Minute_Low=(sysParam.Minute&0x0f);  //分钟的个位
    sysParam.Minute_High=((sysParam.Minute>>4)&0x0f);  //分钟的十位
